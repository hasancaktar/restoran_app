  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(  actions: [IconButton(onPressed: (){
          ...
          title: Text("${service.myMenu.menus![0].items![8].name}"),
        ),
        body: Center(
            child: ListView.builder(
                itemCount: service.myMenu.menus![0].items![8].items!.length,
                itemBuilder: (context, index) {
                  String? images =
                      service.myMenu.menus![0].items![8].items![index].image;
                    ...
                    child: Column(
                      children: [
                        ...
                        GestureDetector(
                          onTap: () {
                            Total.index++;
                            var tt=service.myMenu.menus![0].items![8].items![index].price;
                          ...
                          child: Card(
                            child: ListTile(
                              leading: Image.asset("${"assets" + img}",
                                  fit: BoxFit.cover),
                              title: Text(
                                  "${service.myMenu.menus![0].items![8].items![index].name}"),
                              trailing: Text(
                                  "${service.myMenu.menus![0].items![8].items![index].price} TL"),
                            ),
                          ),
                        ),
